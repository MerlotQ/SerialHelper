...
#endif
{
	...
	partial class LibZResolver
	{
			...
			string libzFileName, bool optional = true)
		{
			try
			{
				...
				myHelpers.Debug(string.Format("Opening library '{0}'", fileName));
			...
			Type assemblyHook, string libzFileName, bool optional = true)
		{
			try
			{
				...
				myHelpers.Debug(string.Format("Opening library '{0}' from '{1}'", libzFileName, resourceName));
		...
		public static void RegisterDecoder(string codecName, Func<byte[], int, byte[]> decoder, bool overwrite = false)
		{
			...
			else
			{
				...
				{
					myHelpers.Warn(string.Format("Codec '{0}' already registered", codecName));
				}
			}
		}
		...
		private static Assembly Resolve(ResolveEventArgs args)
		{
			...
			{
				var name = args.Name;
				myHelpers.Debug(string.Format("Resolving: '{0}'", name));
				var result =
					TryLoadAssembly3(name) ??
						MatchByPartialName(name).Select(TryLoadAssembly3).FirstOrDefault(a => a != null);

				if (result != null)
					myHelpers.Debug(string.Format("Found: '{0}'", name));
				else
					myHelpers.Warn(string.Format("Not found: '{0}'", name));
	...
	namespace Internal
	{
		...
		partial class LibZReader: IDisposable
		{
			...
			{
				if (String.IsNullOrEmpty(codecName))
					throw myHelpers.Error(new ArgumentException("codecName is null or empty.", "codecName"));
				if (decoder == null)
					throw myHelpers.Error(new ArgumentNullException("decoder", "decoder is null."));
				...
				else
				{
					...
					{
						throw myHelpers.Error(new ArgumentException(
		...
		internal class LibZCatalog: ComposablePartCatalog
		{
			...
			private void Initialize()
			{
				...
				foreach (var assemblyName in assemblyNames)
				{
					...
					{
						myHelpers.Error(string.Format("Could not load catalog for '{0}'", assemblyName));
						myHelpers.Error(e);
					}
				}
		...
		public static class myHelpers
		{
			...
			private static readonly Assembly ThisAssembly = typeof(myHelpers).Assembly;
			...
			/// <summary>Initializes the <see cref="myHelpers"/> class.</summary>
			static myHelpers()
			{
				var value =
